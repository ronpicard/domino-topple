...
/**
 * Rebuild a saved layout against its level, keeping only pieces that are still legal: on the table,
 * supported, not overlapping, within the tray inventory. Stored data is untrusted, so each piece is
 * re-resolved rather than taken as is. Pieces are retried until none is added, so one that stands on
 * a piece saved after it still finds its support.
 */
export function restoreLayout(level: LevelDef, placed: PlacedPiece[]): PlacedPiece[] {
  const kept: PlacedPiece[] = []
  let pending = placed.filter((piece) => isPlaceable(piece.kind))
  let added = true
  while (added && pending.length > 0) {
    added = false
